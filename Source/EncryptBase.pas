...
procedure TEncrypt.InitKey(const Key: string; IVector: Pointer);
var
  I: Integer;
begin
  Hash.Init;
...
function TEncrypt.EncodeString(const Source: string): string;
begin
  SetLength(Result, Length(Source));
  EncodeBuffer(PChar(Source)^, PChar(Result)^, Length(Source));
end;

function TEncrypt.DecodeString(const Source: string): string;
begin
  SetLength(Result, Length(Source));
